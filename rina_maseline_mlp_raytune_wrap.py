...
class RINA_Tune():
    ...
    def __init__(self, options):
        ...
        self.phi_net = mlmodel.E2E_Phi_Net(options)
    ...
    def train_model(self):
        ...
        for epoch in range(0, self.epochs):
            ...
            if epoch % 10 == 0 and self.display_progress:
                print('[%d] loss_f: %.2f' % (epoch + 1, running_loss_f / self.num_train_classes))
            ...
            if epoch % self.model_save_freq == 0:
                mlmodel.save_model(phi_net=self.phi_net, h_net=None, modelname=self.modelname + '-epoch-' + str(epoch), options=self.options)

        # Save the model after the final batch of epochs
        mlmodel.save_model(phi_net=self.phi_net, h_net=None, modelname=self.modelname + '-epoch-' + str(self.epochs), options=self.options)

    def save_training_data(self):
        fig, axs = plt.subplots(1, 1)
        
        axs.plot(self.Loss_f, label="train")
        axs.set_xlabel('epoch')
        axs.set_ylabel('f-loss [N]')
        axs.grid()
        axs.set_title('training f loss')
    ...
    def eval_model(self, eval_adapt_start, eval_adapt_end, eval_val_start, eval_val_end, output_path_ims, output_path_txt):
        for i, data in enumerate(self.TestData):
            ...
            mlmodel.vis_validation_e2e(t=data.meta['steps'], x=data.X, y=data.Y, phi_net=self.phi_net, h_net=None, 
        ...
        for data in self.TestData:
            ...
            error_1, error_2, error_3 = mlmodel.error_statistics_hist_e2e(data.X, data.Y, self.phi_net, None, self.options, output_path_ims, image_name)
            print('**** :', data.meta['condition'], '****')
